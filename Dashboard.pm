package Dashboard;

use BOSS::Config;
use Dashboard::TabManager;
use Manager::Dialog qw(Approve);
use MyFRDCSA;

use Data::Dumper;
use File::Slurp;
use PPI::Document;
use PPI::Dumper;
use Tk;
use Tk::NoteBook;
use Tk::Toplevel;

use Class::MethodMaker
  new_with_init => 'new',
  get_set       =>
  [

   qw / Config MyMainWindow MainMenu MyTabManager GUIs MyWindows Withdrawns /

  ];

sub init {
  my ($self,%args) = @_;
  $specification = "
	-u [<host> <port>]	Run as a UniLang agent

	-w			Require user input before exiting
";
  $UNIVERSAL::systemdir = ConcatDir(Dir("minor codebases"),"dashboard");
  $self->Config(BOSS::Config->new
		(Spec => $specification,
		 ConfFile => ""));
  my $conf = $self->Config->CLIConfig;
  if (exists $conf->{'-u'}) {
    $UNIVERSAL::agent->Register
      (Host => defined $conf->{-u}->{'<host>'} ?
       $conf->{-u}->{'<host>'} : "localhost",
       Port => defined $conf->{-u}->{'<port>'} ?
       $conf->{-u}->{'<port>'} : "9000");
  }

  $self->MyTabManager
    (Dashboard::TabManager->new());
}

sub Execute {
  my ($self,%args) = @_;
  my $conf = $self->Config->CLIConfig;
  if (exists $conf->{'-u'}) {
    # enter in to a listening loop
    while (1) {
      $UNIVERSAL::agent->Listen(TimeOut => 10);
    }
  }
  if (exists $conf->{'-w'}) {
    Message(Message => "Press any key to quit...");
    my $t = <STDIN>;
  }
  $self->StartGUI(%args);
}

sub StartGUI {
  my ($self,%args) = @_;
  $self->MyMainWindow
    (
     $args{MainWindow} || MainWindow->new
     (
      -title => "FRDCSA Dashboard",
      -height => 600,
      -width => 800,
     ),
    );
  $UNIVERSAL::managerdialogtkwindow = $self->MyMainWindow;
  $self->MainMenu
    ($self->MyMainWindow->menu
     ());
  $self->MyTabManager->Execute
    (
     MainWindow => $self->MyMainWindow,
     MainMenu => $self->MainMenu,
    );
  $self->MyMainWindow->configure
    (
     -menu => $self->MainMenu,
    );
  MainLoop;
}

sub ProcessMessage {
  my ($self,%args) = @_;
  my $m = $args{Message};
  my $it = $m->Contents;
  if ($it) {
    if ($it =~ /^echo\s*(.*)/) {
      $UNIVERSAL::agent->SendContents
	(Contents => $1,
	 Receiver => $m->{Sender});
    } elsif ($it =~ /^(quit|exit)$/i) {
      $UNIVERSAL::agent->Deregister;
      exit(0);
    }
  }
}

sub ToggleWithdrawn {
  my ($self,%args) = @_;
  my $item = $args{Item};
  if ($args{Withdraw} or ! $self->Withdrawns->{$item}) {
    $self->MyWindows->{$item}->MyMainWindow->withdraw();
    $self->Withdrawns->{$item} = 1;
  } else {
    $self->MyWindows->{$item}->MyMainWindow->deiconify();
    $self->MyWindows->{$item}->MyMainWindow->raise();
    $self->Withdrawns->{$item} = 0;
  }
}

sub Exit {
  my ($self,%args) = @_;
  if (Approve("Exit FRDCSA Dashboard?")) {
    exit(0);
  }
}

1;
